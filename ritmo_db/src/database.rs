...
impl Database {
    ...
    pub async fn backup_to(&self, destination: &PathBuf) -> RitmoResult<()> {
            ...
            .await
        {
            Ok(_) => {}
            Err(e) => {
                return Err(RitmoErr::DatabaseQueryFailed(format!(
                    "Errore nel checkpoint WAL: {}",
                    e
                )))
            }
        }
        ...
        if vacuum_result.is_err() {
            ...
            match fs::copy(&self.path, destination) {
                Ok(_) => {}
                Err(e) => {
                    return Err(RitmoErr::IoError(format!(
    ...
    async fn count_records(pool: &Pool<Sqlite>) -> RitmoResult<RecordCounts> {
        ...
        let books_count: (i64,) = sqlx::query_as("SELECT count(*) FROM books")
            ...
            .await
            .map_err(|e| {
                RitmoErr::DatabaseQueryFailed(format!("Errore nel conteggio dei libri: {}", e))
            })?;
        ...
        let authors_count: (i64,) = sqlx::query_as("SELECT count(*) FROM authors")
            ...
            .await
            .map_err(|e| {
                RitmoErr::DatabaseQueryFailed(format!("Errore nel conteggio degli autori: {}", e))
            })?;
        ...
        let tags_count: (i64,) = sqlx::query_as("SELECT count(*) FROM tags")
            ...
            .await
            .map_err(|e| {
                RitmoErr::DatabaseQueryFailed(format!("Errore nel conteggio dei tag: {}", e))
            })?;
