...
impl Roles {
    pub async fn create(
        ...
        let mut rec = sqlx::query("INSERT INTO roles (name, description) VALUES (?, ?)")
            .bind(name)
            .bind(description)
            .execute(pool)
            .await?;
    ...
    pub async fn get(pool: &sqlx::SqlitePool, id: i64) -> Result<Option<Roles>, sqlx::Error> {
        let result = sqlx::query_as::<_, Roles>(
