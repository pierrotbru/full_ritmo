...
async fn test_database_create() {
    ...
    assert!(result.is_ok(), "Errore nella creazione del database: {:?}", result.err());
    ...
    assert!(result.is_ok(), "Il pool di connessioni non funziona correttamente");
    ...
    for table in tables {
        let result = sqlx::query(&format!("SELECT name FROM sqlite_master WHERE type='table' AND name='{}'", table))
            .fetch_optional(pool)
            .await;
        assert!(result.is_ok(), "Errore nella query per verificare la tabella {}: {:?}", table, result.err());
    ...
    let formats = sqlx::query("SELECT COUNT(*) as count FROM formats")
        .fetch_one(pool)
        .await;
    assert!(formats.is_ok(), "Errore nella query per contare i formati: {:?}", formats.err());
...
async fn test_database_open_non_existent() {
    ...
    assert!(result.is_err(), "Apertura riuscita di un database inesistente");
...
async fn test_database_create_open_cycle() {
    ...
    assert!(create_result.is_ok(), "Errore nella creazione del database: {:?}", create_result.err());
    ...
    assert!(open_result.is_ok(), "Errore nell'apertura del database: {:?}", open_result.err());
...
async fn test_database_metadata_persistence() {
    ...
    let db1 = Database::create(&db_path).await.expect("Impossibile creare il database");
    ...
    // Riapri il database
    let db2 = Database::open(&db_path).await.expect("Impossibile aprire il database");
    
    // Verifica che il timestamp di creazione sia lo stesso, ma quello di aggiornamento sia cambiato
    assert_eq!(db2.get_metadata().created_at, created_at, "Il timestamp di creazione è cambiato");
    assert!(db2.get_metadata().updated_at > created_at, "Il timestamp di aggiornamento non è stato aggiornato");
...
async fn test_database_migration_upgrades() {
    ...
    sqlx::Sqlite::create_database(&db_url).await.expect("Impossibile creare il database");
    let pool = sqlx::SqlitePool::connect(&db_url).await.expect("Impossibile connettersi al database");
    
    // Crea la tabella metadata manualmente
    sqlx::query("CREATE TABLE metadata (key TEXT PRIMARY KEY, value TEXT NOT NULL)")
        .execute(&pool).await.expect("Impossibile creare la tabella metadata");
    ...
    let db = Database::open(&db_path).await.expect("Impossibile aprire il database");
    
    // Verifica che la versione sia stata aggiornata
    assert_eq!(db.get_metadata().version, env!("CARGO_PKG_VERSION"), 
              "La versione non è stata aggiornata");
    assert!(db.get_metadata().updated_at > now, 
